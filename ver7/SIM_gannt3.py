...
class FJSP_simulator():
    ...
    def step(self, action):
        ...
        while True:
            machine, delay = self.check_availability()
            if delay == True:
                s_prime = self.set_state()
                self.event_list.sort(key = lambda x:x[4], reverse = False)
                last_time = self.event_list[0][4]
                reservation_time = self.r_list[machine].reservation_time
                if reservation_time == last_time:
                    pass
                else:
                    self.r_list[machine].delay_setting("delay", reservation_time)
                    r = -(reservation_time - last_time)
                    break
            if machine == "NONE":
                self.process_event()
                if self.num_of_op == 0:
